...
/**
 * Interface adaptada para Auto de Depoimento Directo
 */
interface IData {
  preview: {
    processoNumero?: string;
    numeroFolha?: string;
    label?: string; // Ex: AUTO DE DEPOIMENTO DIRECTO
    dataEmissao?: string;
    ...
    relacoesParentesco?: string; // Campo específico (Linha 125)
    depoimentoConteudo?: string; // Conteúdo principal (Linha 132)
  };
}

export function AutoDepoimentoDirectoTemplate({ preview }: IData) {
  const { user } = useUser();
  ...
  return (
    <div className="rounded-2xl bg-muted/30 p-4 lg:p-8">
      <div className="mx-auto w-full max-w-[794px] bg-white p-12 text-black shadow-lg font-serif">
        {/* Cabeçalho Oficial [cite: 97-100] */}
        <header className="text-center space-y-1">
          ...
          <h3 className="text-lg font-bold uppercase underline mt-4">
            {preview?.label || "AUTO DE DEPOIMENTO DIRECTO"}
          </h3>
        </header>

        <div className="mt-4 text-[14px] leading-relaxed text-justify space-y-5">
          {/* Introdução Local e Data [cite: 104-108] */}
          <p>
            Aos{" "}
            <strong>
              {preview?.dataEmissao
            ...
            onde se achava o
            <strong> {emptyLine(preview?.descricaoOndeSeAchava, 40)}</strong>{" "}
            comigo
          ...
          <p className="indent-8">
            ...
            sala destinada, onde veio a depar sobre os factos constantes dos
            autos, pela ordem e forma que segue.
          </p>

          {/* Identificação da Testemunha [cite: 111-123] */}
          <p>
            Disse chamar-se{" "}
          ...
          <div>
            ...
            <div className="min-h-[60px] border-l border-slate-200 pl-4 whitespace-pre-wrap break-all">
        ...
        <footer className="mt-4">
          ...
          <div className="mt-4 flex justify-between items-end text-[9px] text-slate-400 font-sans">
