"use client";

import { useUser } from "@/hooks/user-context";
import { formatarDataExtenso } from "@/lib/date-utils";
import Image from "next/image";

/**
 * Interface adaptada para Auto de Acareação
 */
interface IData {
  preview: {
    processoNumero?: string;
    numeroFolha?: string;
    dataEmissao?: string;
    cidade?: string;
    unidadeSIC?: string;
    investigadorNome?: string;
    acareadosNomes?: string; // Intervenientes que serão acareados
    outrosPresentes?: string; // Advogados, curadores ou outros oficiais
    conteudoAcareacao?: string; // Relato do confronto e respostas
  };
}

export function AutoAcareacaoTemplate({ preview }: IData) {
  const { user } = useUser();

  const emptyLine = (val?: string, length: number = 20) =>
    val || "_".repeat(length);

  return (
    <div className="rounded-2xl bg-muted/30 p-4 lg:p-8">
      <div className="mx-auto min-h-[1123px] w-full max-w-[794px] bg-white p-12 text-black shadow-lg font-serif">
        {/* Cabeçalho Oficial */}
        <header className="text-center space-y-1">
          <div className="flex justify-center mb-2">
            <Image src="/insignia.jpg" alt="Insígnia" width={55} height={50} />
          </div>
          <h1 className="text-sm font-bold uppercase tracking-widest">
            República de Angola
          </h1>
          <p className="text-xs font-bold">-------***------</p>
          <h2 className="text-sm font-bold uppercase">
            Ministério do Interior
          </h2>
          <h2 className="text-sm font-bold uppercase">
            Serviço de Investigação Criminal
          </h2>

          <div className="pt-6 flex justify-end">
            <span className="text-xs border-b border-black px-2">
              Fls. {preview?.numeroFolha || "_________"}
            </span>
          </div>

          <h3 className="text-lg font-bold uppercase underline mt-4">
            AUTO DE ACAREAÇÃO
          </h3>
        </header>

        <div className="mt-8 text-[14px] leading-relaxed text-justify space-y-6">
          {/* Introdução */}
          <p>
            Aos{" "}
            <strong>
              {preview?.dataEmissao
                ? formatarDataExtenso(new Date(preview.dataEmissao))
                : "__________ dias do mês de __________________ do ano__________"}
            </strong>
            , nesta cidade de{" "}
            <strong className="break-all">
              {emptyLine(preview?.cidade, 25)}
            </strong>{" "}
            e S.I.C, onde se achava o
            <strong className="break-all">
              {emptyLine(preview?.unidadeSIC, 50)}
            </strong>
            . Comigo
            <strong>
              {" "}
              {preview?.investigadorNome ||
                user?.name ||
                "_________________________________________"}
            </strong>
            .
          </p>

          {/* Intervenientes Acareados */}
          <p>
            Aqui compareceram para serem acareados os intervenientes:
            <strong className="break-all">
              {" "}
              {emptyLine(preview?.acareadosNomes, 80)}
            </strong>
            .
          </p>

          {/* Outros Presentes (Advogados/Oficiais) */}
          <p>
            Igualmente presentes,{" "}
            <strong className="break-all">
              {emptyLine(preview?.outrosPresentes, 80)}
            </strong>
            .
          </p>

          {/* Procedimento de Acareação */}
          <p className="indent-8">
            Que lidas as declarações e/ou depoimentos prestados nos autos, foram
            os acareados colocados frente a frente para as questionarem ou
            impugnarem.
          </p>

          {/* Corpo da Acareação */}
          <div className="min-h-[50px] border-l-2 border-slate-100 pl-4 italic whitespace-pre-wrap leading-relaxed break-all">
            {emptyLine(preview?.conteudoAcareacao, 280)}
          </div>

          <p className="pt-4">
            E para constar lavrou-se o presente auto que vai ser devidamente
            assinado.
          </p>
        </div>

        {/* Assinaturas */}
        <footer className="mt-10">
          <div className="grid grid-cols-2 gap-8 text-center text-[10px] font-bold uppercase">
            <div className="space-y-4">
              <div className="border-b border-black w-full" />
              <p>OS ACAREADOS</p>
            </div>
            <div className="space-y-4">
              <div className="border-b border-black w-full" />
              <p>O INVESTIGADOR CRIMINAL</p>
            </div>
            <div className="space-y-4 col-span-2 w-1/2 mx-auto mt-4">
              <div className="border-b border-black w-full" />
              <p>OUTROS PRESENTES / ADVOGADOS</p>
            </div>
          </div>

          <div className="mt-10 flex justify-between items-end text-[9px] text-slate-400 font-sans italic">
            <span>Mod. 50</span>
            <span className="uppercase font-bold text-black/20 tracking-widest">
              SIC - Documento Oficial
            </span>
          </div>
        </footer>
      </div>
    </div>
  );
}
