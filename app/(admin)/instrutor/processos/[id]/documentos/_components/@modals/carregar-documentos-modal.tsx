...
import { PecaCategory, UploadDocumento } from "../../_utils/types";
import UploadDocumentosModal from "./upload-documentos-modal";

export default function CarregarDocumentosModal() {
  ...
  const [formData, setFormData] = useState<UploadDocumento>({
    titulo: "",
    tipo: "",
    descricao: "",
    arquivo: null,
  ...
  const handleTypeSelection = (typeLabel: string, url: string) => {
    setFormData((prev) => ({
      ...prev,
      tipo: typeLabel,
          ...
          <DialogContent className="sm:max-w-4xl p-0 overflow-hidden max-h-[90vh]">
            <DialogHeader className="p-6 pb-2">
              <DialogTitle className="text-2xl">
                Carregar Tipo de Documento
              </DialogTitle>
              <DialogDescription>
                Escolha o tipo de documento que deseja carregar ao processo.
              </DialogDescription>
              <Input
                value={query}
